...
export const logout = (authUserId) =>
  userLock((resolve, reject) => {
...
export const assertValidChannel = (channelId) =>
...
export const getChannel = (authUserId, channelId) =>
  channelLock((resolve, reject) => {
...
export const addChannel = (authUserId, name, priv, description) =>
  channelLock((resolve, reject) => {
    if (name === undefined) {
    ...
    const payload = newChannelPayload(name, authUserId, priv, description);
...
export const updateChannel = (authUserId, channelId, name, description) =>
  channelLock((resolve, reject) => {
    assertChannelMember(authUserId, channelId);
    if (name !== undefined) {
...
export const joinChannel = (authUserId, channelId) =>
  channelLock((resolve, reject) => {
...
export const leaveChannel = (authUserId, channelId) =>
  channelLock((resolve, reject) => {
    assertChannelMember(authUserId, channelId);
...
export const inviteChannel = (authUserId, channelId, userId) =>
  channelLock((resolve, reject) => {
    assertChannelMember(authUserId, channelId);
...
export const updateProfile = (authUserId, email, password, name, bio, image) =>
  userLock((resolve, reject) => {
    if (name || name === "") {
...
const findMsgIdxInChannel = (chId, mId) => {
  const index = channels[chId].messages.findIndex(
    (m) => m.id === parseInt(mId, 10),
  );
  if (index === -1) {
    throw new InputError(
      "No message with this message ID exists in this channel",
    );
  }
  return index;
};
...
// start is an int
export const getMessages = (authUserId, channelId, start) =>
  channelLock((resolve, reject) => {
...
export const sendMessage = (authUserId, channelId, message, image) =>
  channelLock((resolve, reject) => {
    if (
      isInvalidMessage(message) ||
      isInvalidMessage(image) ||
      (message === undefined && image === undefined)
    ) {
    ...
    channels[channelId].messages.unshift(newMessage);
    resolve({ ...newMessage, reacts: [...newMessage.reacts] });
  });

export const deleteMessage = (authUserId, channelId, messageId) =>
  channelLock((resolve, reject) => {
    assertChannelMember(authUserId, channelId);
    const messageIndex = findMsgIdxInChannel(channelId, messageId);
    if (
      channels[channelId].messages[messageIndex].sender !==
      parseInt(authUserId, 10)
    ) {
      return reject(
        new AccessError("You can't delete a message you didn't send"),
      );
    }
...
export const editMessage = (
  ...
  channelLock((resolve, reject) => {
    if (
      isInvalidMessage(newMessage) ||
      isInvalidMessage(newImage) ||
      (newMessage === undefined && newImage === undefined)
    ) {
      return reject(new InputError("Invalid message or image"));
    }
    assertChannelMember(authUserId, channelId);
    ...
    if (
      ...
      return reject(
        new AccessError("You can't edit a message you didn't send"),
      );
    }
    ...
    channels[channelId].messages[messageIndex].editedAt =
      new Date().toISOString();
    resolve();
  });

export const pinMessage = (authUserId, channelId, messageId) =>
  channelLock((resolve, reject) => {
    assertChannelMember(authUserId, channelId);
    const messageIndex = findMsgIdxInChannel(channelId, messageId);
...
export const unpinMessage = (authUserId, channelId, messageId) =>
  channelLock((resolve, reject) => {
    assertChannelMember(authUserId, channelId);
    const messageIndex = findMsgIdxInChannel(channelId, messageId);
    if (!channels[channelId].messages[messageIndex].pinned) {
...
export const reactMessage = (authUserId, channelId, messageId, react) =>
  channelLock((resolve, reject) => {
    if (react === undefined) {
    ...
    const messageIndex = findMsgIdxInChannel(channelId, messageId);
    if (
      channels[channelId].messages[messageIndex].reacts.findIndex(
...
export const unreactMessage = (authUserId, channelId, messageId, react) =>
  channelLock((resolve, reject) => {
    if (react === undefined) {
      return reject(new InputError("No react value provided"));
    }
    assertChannelMember(authUserId, channelId);
    const messageIndex = findMsgIdxInChannel(channelId, messageId);
    const reactIndex = channels[channelId].messages[
      messageIndex
    ].reacts.findIndex(
      (r) => r.user === parseInt(authUserId, 10) && r.react === react,
    );
    if (reactIndex === -1) {
      return reject(
        new InputError(
          "This message does not contain a react of this type from this user",
        ),
      );
    }
